...
"""
core/validation.py
Lógica de validação de correspondência entre produto do feed e produto da loja.
"""
...
def validate_product_match(our_parts: List[str],
                           page_identifiers: Dict[str, List[str]],
                           page_url: str,
                           page_text: str) -> ValidationResult:
    """
    ...
    if our_main_ref and our_main_ref in page_url_norm:
        return ValidationResult(True, 0.90, MatchType.EXACT_MATCH, [our_main_ref], f"Ref principal no URL: {our_main_ref}")
...
def extract_codes_from_text(text: str, min_length: int = 4) -> List[str]:
    """
    Wrapper para extract_alphanumeric_codes com nome antigo.
    Mantido por compatibilidade.
    """
    return extract_alphanumeric_codes(text, min_length=min_length)
